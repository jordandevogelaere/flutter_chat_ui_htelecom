...
Color getUserAvatarNameColor(types.User user, List<Color> colors) => colors[user.id.hashCode % colors.length];
...
bool isConsistsOfEmojis(
  ...
  final singleEmojiRegExp = RegExp(multiEmojiRegExp.pattern.replaceFirst(r'+$', r'$'));
  ...
  for (var i = messages.length - 1; i >= 0; i--) {
      ...
      final isFirstInGroup = notMyMessage &&
          ((message.author.id != previousMessage?.author.id) ||
              (messageHasCreatedAt &&
                  ...
                  message.createdAt!.millisecondsSinceEpoch - previousMessage!.createdAt!.millisecondsSinceEpoch >
    ...
    if (messageHasCreatedAt && nextMessageHasCreatedAt) {
      nextMessageDateThreshold =
          nextMessage!.createdAt!.millisecondsSinceEpoch - message.createdAt!.millisecondsSinceEpoch >=
              dateHeaderThreshold;

      nextMessageDifferentDay = message.createdAt!.day != message.createdAt!.day;
    ...
    if (message.id == lastReadMessageId && !isLast) {
      chatMessages.insert(
        0,
        UnreadHeaderData(
          marginTop: nextMessageDifferentDay || nextMessageDateThreshold ? 0 : 8,
        ),
      );
    }
  }
